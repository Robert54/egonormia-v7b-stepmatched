#!/bin/bash
# Portable launcher for the v6b natural-language-output plus sensibility-generation SFT run.
...
TEMPLATE="${ROOT_DIR}/configs/egonormia_sft_v6b.toml.template"
...
RUN_NAME="egonormia_sft_v6b_seed${SEED}"
EXPERIMENT_NAME="egonormia_sft_v6b_nloutput_seed${SEED}"
...
sed \
  ...
  -e "s/__EXPERIMENT_NAME__/$(escape_sed "${EXPERIMENT_NAME}")/" \
  -e "s/__SEED__/${SEED}/" \
  "${TEMPLATE}" > "${TMP_CONFIG}"

echo "Starting v6b SFT..."
